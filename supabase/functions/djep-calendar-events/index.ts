...
async function firecrawlScrape(): Promise<{ events: DjepEvent[]; raw: any }> {
  ...
  const body = {
    ...
    actions: [
      ...
      { type: "write", selector: "input[name='username']", text: DJEP_USERNAME ?? "" },
      { type: "write", selector: "input[name='password']", text: DJEP_PASSWORD ?? "" },
      { type: "click", selector: "input[type='submit'][name='submit']" },
      { type: "wait", milliseconds: 4000 },
      { type: "executeJavascript", script: `(() => ({ stage: 'post-login', title: document.title, url: location.href, bodyStart: (document.body.innerText || '').slice(0, 600) }))()` },
